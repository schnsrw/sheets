...
| Release tag form | `v0.3.2` | `@casualoffice/sheets@0.9.0` |
...
The `schnsrw` → `casualoffice` rename is **complete on both lines**:
...
- **SDK — `@casualoffice/sheets@0.9.0` published** (first publish under the new
  scope). It's the Excalidraw-model restructure (full editor, formula engine,
  `CasualSheetsAPI`, `onChange`, lazy plugins, light/dark) — an early `0.x`, the
  restructure continues. The **old** `@schnsrw/casual-sheets@0.8.0` is a frozen
  pre-restructure line and does **not** have the new API; depend on
  `@casualoffice/sheets`.
