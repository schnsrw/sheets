...
/**
 * Single-row Office-style toolbar. Each group lays out its controls in
 * one horizontal row so icons share a baseline. Less-common features
 * live in the top MenuBar; panel toggles live in the right-side rail
 * (see PanelRail.tsx) so the toolbar doesn't have to host them.
 *
 * Paste Special is reachable from the Clipboard group via a small icon
 * next to Cut/Copy; the dialog itself lives in MenuBar (single source
 * of truth for modal state).
 */
  ...
  return (
    <nav className="toolbar" data-testid="toolbar" aria-label="Toolbar">
      ...
      <div className="toolbar__inner" ref={innerRef}>
        ...
        {/* ── Clipboard — Paste is the BIG primary button. Cut / Copy /
              Paste Special / Format Painter stack as small buttons. */}
        <RibbonGroup
          label="Clipboard"
          lead={
            <BigToolbarButton
        ...
        {/* ── Font — name + size + A↑ / A↓ on row 1, B I U S + colour +
              fill + borders on row 2. */}
        <RibbonGroup
          label="Font"
          row1={<>
            <ToolbarSelect
        ...
        {/* ── Number — format dropdown row 1, quick toggles row 2. */}
        <RibbonGroup
          label="Number"
          row1={
            <ToolbarSelect
        ...
        {/* ── Insert — AutoSum + Chart + PivotTable + Format as Table on
              row 1. Sort & Filter + Link + Comment on row 2. Chart and
              Pivot dispatch a CustomEvent that MenuBar listens for to
              open the existing InsertChartDialog / InsertPivotDialog. */}
        <RibbonGroup
          label="Insert"
          row1={<>
            <ToolbarDropdown
